...
import PourVousStrip from "@/app/schedule/PourVousStrip";
...
export default async function StudioSchedulePage({
  ...
  const [locations, classTypes] = await Promise.all([
    ...
    db.classType.findMany({ where: { studioId, active: true }, orderBy: { name: "asc" } }),
  ]);
  ...
  if (user) {
    ...
    const recentBookings = await db.booking.findMany({
      where: {
        userId: user.id,
        status: "CONFIRMED",
        session: { studioId, startTime: { gte: since60 } },
      },
  ...
  const base = `/studio/${params.slug}/schedule`;
  ...
  const buildHref = (v: "day" | "week" | "grid") => {
    ...
    return `${base}?view=${v}${dateParam}${locParam}`;
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="section-title">{studio.name}</p>
      ...
      <div className="flex items-center justify-between gap-2 bg-white border border-stone2-200 p-1.5">
        <Link
          href={`${base}?view=${view}&date=${prevDate}${locationFilter ? `&location=${locationFilter}` : ""}`}
        ...
        <div className="flex gap-0.5 overflow-x-auto">
          {strip.map((d) => (
            <Link
              key={d.iso}
              href={`${base}?view=${view}&date=${d.iso}${locationFilter ? `&location=${locationFilter}` : ""}`}
        ...
        <Link
          href={`${base}?view=${view}&date=${nextDate}${locationFilter ? `&location=${locationFilter}` : ""}`}
          className="btn-ghost px-4 min-h-[44px] flex items-center"
        >
          →
        </Link>
