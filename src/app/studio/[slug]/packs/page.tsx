...
import PurchaseButton from "@/app/packs/PurchaseButton";

export default async function StudioPacksPage({
  params,
  searchParams,
}: {
  params: { slug: string };
  ...
  const [user, packs] = await Promise.all([
    getCurrentUser(),
    db.plan.findMany({
      where: { studioId: studio.id, type: "CREDIT_PACK", active: true },
  ...
  return (
    <div className="space-y-10">
      ...
      {packs.length === 0 && (
        <div className="border border-stone2-100 bg-white p-12 text-center">
          <p className="text-stone2-400 text-sm">Aucun pack disponible pour le moment.</p>
        </div>
      )}
